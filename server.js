  ...
  for (const trade of [...activeTrades]) {
    try {
      ...
      const candles = d.values
        .filter(c => {
          // Twelve Data retourne les datetimes sans timezone
          // On force UTC explicitement en remplaçant l'espace par T et ajoutant Z
          const dt = new Date(c.datetime.replace(' ', 'T') + 'Z').getTime();
          return dt >= entryTime;
        })
