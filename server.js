...
function getParisHour() {
  const now = new Date();
  // Détecter heure d'été (dernier dimanche mars → dernier dimanche octobre)
  const year = now.getUTCFullYear();
  const dstStart = new Date(Date.UTC(year, 2, 31 - (new Date(year, 2, 31).getUTCDay() || 7) % 7, 1));
  const dstEnd   = new Date(Date.UTC(year, 9, 31 - (new Date(year, 9, 31).getUTCDay() || 7) % 7, 1));
...
function getParisDay() {
  ...
  const parisMs = now.getTime() + offset * 3600000;
  return new Date(parisMs).getUTCDay(); // 0=dim, 1=lun ... 6=sam
}
...
function isMarketOpen() {
  ...
  // Dimanche → fermé jusqu'à 22h Paris
  if (day === 0 && hour < 22) return false;
  // Vendredi → fermé à partir de 22h Paris
  if (day === 5 && hour >= 22) return false;
  return true;
}

// ─── FRÉQUENCE DU SCAN ────────────────────────────────────────────────────────
// 15 min entre 8h-22h Paris | 1h entre 22h-8h Paris | 0 le weekend
function getScanInterval() {
  const hour = getParisHour();
  if (hour >= 8 && hour < 22) return 15 * 60 * 1000;  // 15 minutes
...
async function start() {
  ...
  // Scheduling dynamique : 15 min entre 8h-22h, 1h entre 22h-8h
  function scheduleNext() {
    const interval = getScanInterval();
    const minutes = Math.round(interval / 60000);
    console.log(`⏱  Prochain scan dans ${minutes} minute(s)`);
    setTimeout(async () => {
      await runScan();
      scheduleNext();
    }, interval);
