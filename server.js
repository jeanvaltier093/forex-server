...
function computeSignalAnalyzer(candles, pair) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  SIGNAL SELL — les 4 conditions doivent être actives simultanément
  // ════════════════════════════════════════════════════════════════════════
  if (macdDiv === -1 && stochOB && macdMomNeg && nearRes) {
    return {
      pair, direction: 'SELL',
      entryPrice: price.toFixed(dec),
      sl: (price + slPips).toFixed(dec),
      ...
      reliability: 59,
      reasons: [
        'Divergence MACD baissière (prix ↑, MACD ↓)',
        'Stochastique en surachat (K > 80)',
        'Momentum MACD négatif (histogramme décroissant)',
        'Prix proche de la résistance',
        'Combo backtestée 59% / 81 trades / WF Test 56%'
      ],
      engine: 'ANALYZER',
      timestamp: new Date().toISOString()
    };
...
app.get('/analyser', (req, res) => {
  const path = require('path');
  res.sendFile(path.join(__dirname, 'analyser.html'));
});

app.get('/analyzer-ultra', (req, res) => {
  const path = require('path');
  res.sendFile(path.join(__dirname, 'analyzer_ultra.html'));
});

app.get('/deep-analysis', async (req, res) => {
  const apiKey = req.query.key || process.env.TWELVE_DATA_API_KEY;
  const outputsize = parseInt(req.query.outputsize) || 2200;
  const mt = parseInt(req.query.mt) || 80;
  const mwr = parseInt(req.query.mwr) || 55;
  const result = await exhaustive.start(apiKey, outputsize, mt, mwr);
  res.json(result);
});

app.get('/deep-results', (req, res) => {
  const data = exhaustive.getStatus();
  ...
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="refresh" content="${data.running?'10':'0;url=/deep-results'}"><title>Deep Results</title>
<style>body{background:#080c14;color:#e8edf5;font-family:monospace;padding:20px}h1{color:#ffd700}h2{color:#00d4ff}.ok{color:#00ff88}.warn{color:#ffd700}.bad{color:#ff4466}.card{background:#0d1320;border:1px solid #1e2d45;border-radius:8px;padding:16px;margin:10px 0}</style>
</head><body>
<h1>🔬 Deep Exhaustive Analysis</h1>
<div class="card">
<p>Status: <b class="${data.running?'warn':'ok'}">${data.running?'🔄 En cours':'✅ Terminé'}</b></p>
<p>Phase: ${data.phase||'—'}</p>
<p>Détail: ${data.detail||'—'}</p>
<p>Temps écoulé: ${elapsed}s</p>
${data.finishedAt?'<p>Terminé à: '+data.finishedAt+'</p>':''}
${data.running?'<p style="color:#ffd700">⟳ Page se rafraîchit toutes les 10s</p>':''}
</div>
${r?`<h2>TOP 5 BUY</h2>${(r.buy||[]).map((c,i)=>`<div class="card"><b>${i+1}. ${c.names.join(' + ')}</b> [${c.mode}]<br>WR: <span class="ok">${c.wr}%</span> | Trades: ${c.total} | Pips/trade: ${c.avgPips}<br>WF Train: ${c.wfTrain?c.wfTrain.wr+'%':'—'} | <b>WF Test: <span class="${c.wfTest&&c.wfTest.wr>=55?'ok':c.wfTest&&c.wfTest.wr>=45?'warn':'bad'}">${c.wfTest?c.wfTest.wr+'%':'—'}</span></b></div>`).join('')}
<h2>TOP 5 SELL</h2>${(r.sell||[]).map((c,i)=>`<div class="card"><b>${i+1}. ${c.names.join(' + ')}</b> [${c.mode}]<br>WR: <span class="ok">${c.wr}%</span> | Trades: ${c.total} | Pips/trade: ${c.avgPips}<br>WF Train: ${c.wfTrain?c.wfTrain.wr+'%':'—'} | <b>WF Test: <span class="${c.wfTest&&c.wfTest.wr>=55?'ok':c.wfTest&&c.wfTest.wr>=45?'warn':'bad'}">${c.wfTest?c.wfTest.wr+'%':'—'}</span></b></div>`).join('')}`:''}
</body></html>`);
});
