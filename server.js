...
function computeSignalAnalyzer(candles, pair) {
  ...
  // MACD Cross : histogram = macdLine - signalLine, donc son signe suffit
  // (macd.sl n'est pas retourné par calcMACDFull → était toujours undefined)
...
async function fetchCandles(pair) {
  try {
    ...
    // .slice(0, -1) : on retire la dernière bougie (en cours, non clôturée)
...
async function checkTrades() {
  ...
  for (const trade of [...activeTrades]) {
    try {
      ...
      const d = await r.json();
      if (!d.price || d.status === 'error') {
        console.log(`⚠️  ${trade.pair} — prix indisponible`);
      ...
      if (closed) {
        ...
        changed = true;
        console.log(`${result==='WIN'?'✅ GAIN':'❌ PERTE'} ${trade.pair} ${trade.direction} — ${pips>0?'+':''}${pips} pips | prix: ${cur}`);
      } else {
        const distTP  = Math.abs(cur - tp) / pipDiv;
        const distSL  = Math.abs(cur - sl) / pipDiv;
        console.log(`⏸  ${trade.pair} ${trade.direction} | prix: ${cur} | TP: ${tp} (${distTP.toFixed(0)}p) | SL: ${sl} (${distSL.toFixed(0)}p)`);
      }
...
async function runScan() {
  ...
  // Pas de nouveaux signaux le vendredi après 14h Paris
  {
    ...
    if (parisDay === 5 && parisHour >= 14) {
      ...
      await loadCloud(); // sync cloud avant de vérifier les trades
      await checkTrades();
      return;
    }
  }
  ...
  setInterval(async () => {
    for (const [name, url] of targets) {
      try {
        ...
        console.log(`⚠ Keep-alive ${name} échoué: ${e.message}`);
      }
    }
  }, 10 * 60 * 1000);
