...
async function fetchData(apiKey, outputsize) {
  ...
  const snaps = [];
  console.log(`📥 Téléchargement ${PAIRS.length} paires, outputsize=${outputsize}`);
  for (const pair of PAIRS) {
    console.log(`  → ${pair}...`);
    try {
      const fetchPromise = fetch(`https://api.twelvedata.com/time_series?symbol=${pair}&interval=4h&outputsize=${outputsize}&apikey=${apiKey}`);
      const timeoutPromise = new Promise((_,rej) => setTimeout(() => rej(new Error('timeout')), 10000));
      const r = await Promise.race([fetchPromise, timeoutPromise]);
      const d = await r.json();
      if (!d.values || d.status==='error') { await sleep(400); continue; }
